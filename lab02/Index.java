...
public class Index {
    ...
    public static class WordCountMap extends Mapper<Text, Text, Text, Text> {
	    ...
	    int position = 0; String temp; String wordString;
            while (matcher.find()) {
                word.set(matcher.group());
		wordString = word.toString();
		if (contained.containsKey(wordString)) {
		    temp = contained.get(wordString);
		    temp += "," + position;
		    contained.remove(wordString);
		    contained.put(wordString, temp);
		} else {
		    temp = key.toString() + ": " + position;
		    contained.put(wordString, temp);
		}
		position++;
            }
	    for (String wordKey : contained.keySet()) {
		context.write(new Text(wordKey), new Text(contained.get(wordKey)));
	    }	
        }
    }
